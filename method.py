...
def find_dots(conditions, h, a, M, mp):
    dots = dict()

    # TODO: rename
    M_dots = {
        Borders.UPPER: [(0, y) for y in mp.linspace(0, a, M+2)[1:-1]],
        Borders.RIGHT: [(x, a) for x in mp.linspace(0, h, M+2)[1:-1]],
        Borders.BOTTOM: [(h, y) for y in mp.linspace(0, a, M+2)[1:-1]],
        Borders.LEFT: [(x, 0) for x in mp.linspace(0, h, M+2)[1:-1]],
    }

    for border in conditions:
        dots[border] = dict()
        for name in conditions[border]:
            dots[border][name] = M_dots[border]
